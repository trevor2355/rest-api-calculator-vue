<template>
  <b-modal :id='infoModal.id.toString()' :title="'Edit Record: ' + infoModal.id" ok-variant='danger' ok-only ok-title='Cancel' ref='edit-modal' @hide='hide'>
    <div>
    <b-form>
      <b-form-group
        id="input-group-1"
...
export default {
  ...
  methods: {
    ...
    async updateRecord() {
      ...
      let response = await fetch(`http://localhost:3000/api/records/${this.form.id}`, options)
