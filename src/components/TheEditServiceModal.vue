<template>
  <b-modal :key='editModal.id.toString()' :id='editModal.id.toString()' :title="'Edit Record: ' + editModal.content.type" ok-variant='danger' ok-only ok-title='Cancel' ref='edit-modal' @hide='hide'>
    <div>
    <b-form>
      <b-form-group
        id="input-group-1"
  ...
  methods: {
    ...
    onSubmit(evt) {
      ...
      this.updateUser()
        ...
        .catch(err => {
          err
          alert('There was an Error Updating Balance of User: ' + this.editModal.type)
        }) 
    ...
    async updateUser() {
      ...
      let response = await fetch(`http://localhost:3000/api/services/${this.editModal.id}`, options)
